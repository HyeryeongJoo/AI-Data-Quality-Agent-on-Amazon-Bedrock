...
| LLM Analyzer | ⌈의심항목 ÷ 50⌉회 | 항상 (의심항목 > 0) | PRIMARY 분석: **한 번의 응답**에서 `is_error`, `confidence` (HIGH/MEDIUM/LOW), `suggested_correction`을 동시에 반환 |
| **합계 (캐시 MISS, 의심 ≤ 50건)** | **3회** | — | — |
| **합계 (캐시 HIT, 의심 ≤ 50건)** | **1회** | — | — |

> **신뢰도 판정에 대한 주의사항**: 신뢰도(HIGH/MEDIUM/LOW)는 **별도 LLM 호출이 아닙니다.** 명시적인 신뢰도 판정 기준이 PRIMARY 시스템 프롬프트에 포함되어 있어, LLM이 오류 여부와 신뢰도를 동일한 호출에서 함께 판단합니다. 신뢰도를 위한 추가 호출은 없습니다.
